package ax.test.WindowsPipeCommunicatorTestSuite;

import ax.test.CommunicationTestUtil;
import org.junit.jupiter.api.*;
import ax.src.WindowsPipeCommunicator;
...
/*************************************************************************************************
 *  Behavior Driven Testing (BDT) of sendRequest method in WindowsPipeCommunicator class.        *
...
class sendRequestTests {
    ...
    void timeOutTruePipeNotExistsSingleThreadTest(){
        ...
        String response = WindowsPipeCommunicator.sendRequestByPipe(request, true, maxTime);
    ...
    void timeOutPipeNotExistsMultipleThreadTest(){
        ...
        for (String s : getRequestList) {
            ...
            Thread t = new Thread(() -> actualDataMap.put(s, WindowsPipeCommunicator.sendRequestByPipe(request, true,maxTime)));
            threadList.add(t);
        }
    ...
    void timeOutFalseSingleRequestTest(){
        ...
        String response = KeyValueString.getResponseBody(WindowsPipeCommunicator.sendRequestByPipe(request, false, 1));
    ...
    void multiThreadRequestGetDifferentDataTest(){
        ...
        for (int i = 0; i< getRequestList.size(); i++){
            ...
            Thread t = new Thread(()-> {
                String response = KeyValueString.getResponseBody(WindowsPipeCommunicator.sendRequestByPipe(request, false,50));
    ...
    void multiThreadRequestPostDifferentDataTest(){
        ...
        for (String s : postRequestList) {
            ...
            Thread t = new Thread(() -> {
                String response = KeyValueString.getResponseBody(WindowsPipeCommunicator.sendRequestByPipe(requestBody, false, 50));
    ...
    void invalidDataRequest(){
        // null input
        assertThrows(IllegalArgumentException.class, ()-> WindowsPipeCommunicator.sendRequestByPipe(null, false,50));

        //empty String input
        assertThrows(IllegalArgumentException.class, ()-> WindowsPipeCommunicator.sendRequestByPipe("", false,50));
